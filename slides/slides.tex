...
% ---------- school logo on every slide (top-right corner) ----------
\addtobeamertemplate{background canvas}{}{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north east, xshift=-0.35cm, yshift=-0.25cm]
      at (current page.north east)
      {\includegraphics[height=0.85cm]{figs/school_logo.png}};
  \end{tikzpicture}%
}
...
\author[Peng, Yang, Lu]{Jianwei Peng 12531290\and Han Yang 12532527\and Ziling Lu 12542038}
\institute{SUSTech}
...
\begin{frame}{Spectrum of robust controllers}
  ...
  \begin{tabular}{lll}
    ...
    SMC \cite{utkin2009}           & Robust to bounded unc. & Chattering\\
...
\begin{frame}{Tracking error and linearisation}
  ...
  $\xi=(x_e,y_e,\theta_e,v_e,\omega_e)^{\!\top}$:
  \[
    \begin{bmatrix}x_e\\y_e\end{bmatrix}
      = R(\theta)^{\!\top}\!\begin{bmatrix}x_r-x\\y_r-y\end{bmatrix},\ \
    \theta_e=\mathrm{wrap}(\theta_r-\theta),\ \
    v_e=v_r-v,\ \omega_e=\omega_r-\omega.
  \]

  \textbf{Linearisation around} $\xi=0$ with feed-forward $\tau_{ff}$:
  $\ \dot\xi = A\xi + B(u + d)$, where $d$ is the \alert{matched equivalent
  disturbance} — model mismatch and $\tau_d$ projected onto the input channel.

  \begin{block}{Numerical matrices for our parameters}
    \centering\footnotesize
    \setlength{\arraycolsep}{3pt}\renewcommand{\arraystretch}{0.95}
    $A=\!\begin{bmatrix}
       0 & 0.2 & 0 & 1 & 0\\
       -0.2 & 0 & 0.4 & 0 & 0\\
       0 & 0 & 0 & 0 & 1\\
    ...
    \end{bmatrix},\;\;
    \mathrm{rank}\,\mathcal{C}=5\ \Rightarrow\ \text{controllable.}$
  \end{block}
\end{frame}
...
\begin{frame}{Trajectory under enhanced piecewise disturbance}
  \begin{columns}[T]
    ...
    \column{0.42\linewidth}
      \textbf{Setup}
      \begin{itemize}
        \item Robot starts $0.22$\,m off-track at $(2.20,0.10)$
        \item Enhanced $\tau_d$ ($6{-}8\times$ the table values)
              for clear visual contrast
      \end{itemize}

      \vspace{0.4em}
      \textbf{Result at $t=30$\,s}
      \begin{itemize}
        \item LQR end \alert{$0.42$\,m off} reference
        \item LQR+DOB end \alert{$<1$\,mm} from reference
        \item Full-run RMSE: $0.45$ vs $0.10$~m
      \end{itemize}

      \vspace{0.3em}
      LQR trace contracts inside the circle whenever a disturbance
      is active; DOB cancels each one online.
  \end{columns}
\end{frame}
...
\begin{frame}[allowframebreaks]{References}
  ...
  \begin{thebibliography}{99}
    ...
    \bibitem{kayacan2015} E.~Kayacan \textit{et al.},
      ...
      for UGVs,'' \emph{IEEE/ASME TMech}, vol.~21, no.~2, 2016.
    \bibitem{utkin2009} V.~Utkin \textit{et al.},
